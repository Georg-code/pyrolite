...
const createStrikethroughStyleStrategy = () => {
  ...
  return {
    ...
    findDelimiterRanges: (
      ...
      const text = block.getText();
      let strikethroughDelimiterRanges: any[] = [];
      styleRanges.forEach((styleRange: any[]) => {
        ...
        ).map((indices: any[]) => indices.map((x: any) => x + styleRange[0]));
        strikethroughDelimiterRanges =
          strikethroughDelimiterRanges.concat(delimiterRange);
      });
